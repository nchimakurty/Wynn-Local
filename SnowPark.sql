CREATE OR REPLACE PROCEDURE {{EDW_DB_NAME}}.CIPINTEGRATIONODS.SP_CX_IB_EBH_HOTEL_RESV_LOAD()
RETURNS STRING
LANGUAGE PYTHON
RUNTIME_VERSION = '3.9'
PACKAGES = ('snowflake-snowpark-python==1.14.0', 'requests==2.31.0', 'snowflake==0.6.0', 'anyjson==0.3.3')
HANDLER = 'main'
EXTERNAL_ACCESS_INTEGRATIONS = ({{EDW_DB_NAME}}_CX_IB_MONGODB_EXT_RULE)
SECRETS = ('API_CONFIG'={{EDW_DB_NAME}}.TGT_STG_CIPEDW.CX_IB_MONGODB_CONFIG)
COMMENT='STORED PROCEDURE TO GET EBH RESERVATIONS FOR ITINERARY BUILDER'
EXECUTE AS OWNER
AS $$
...
def main(session: snowpark.Session):
    ...
    clear_log_query = "DELETE FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_LOG_TBL WHERE SP_NAME = 'SP_CX_IB_EBH_HOTEL_RESV_LOAD'"
    session.sql(clear_log_query).collect()
    
    try:

        getLastBatchStatusSql = "SELECT MAX(BATCH_ID) MAX_BATCH_ID FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS WHERE BATCH_STATUS='R' AND SITE_ID='EBH'"
        ...
        if lastBatchIdVal is not None:
            updateLastBatchStatusSql = "UPDATE {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS SET BATCH_STATUS='F' WHERE BATCH_STATUS='R' AND SITE_ID='EBH'"
            session.sql(updateLastBatchStatusSql).collect()
        
        clearBatchStatusSql = "DELETE FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS WHERE SITE_ID='EBH' AND BATCH_END_TS < CURRENT_DATE()- INTERVAL '7 days'"
        session.sql(clearBatchStatusSql).collect()

        deleteLastBatchLoadSql = f"TRUNCATE TABLE {{EDW_DB_NAME}}.CIPINTEGRATIONODS.STG_CX_IB_EBH_HOTEL_RESV_LOAD"
        ...
        createNewBatchSql = f"""INSERT INTO {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS (SITE_ID, BATCH_ID, BATCH_STATUS, EXTRACT_FROM_TS, EXTRACT_TO_TS, EXTRACT_COUNT, BATCH_START_TS, BATCH_END_TS)
                                (SELECT 'EBH', TO_CHAR(CURRENT_TIMESTAMP(), 'YYYYMMDDHH24MISS'), 'R', (SELECT COALESCE(MAX(EXTRACT_TO_TS),(CURRENT_TIMESTAMP() - INTERVAL '1 minute')) EXTARCT_TO_TS 
                                FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS WHERE BATCH_STATUS='C' AND SITE_ID='EBH'), NULL, NULL, CURRENT_TIMESTAMP(), NULL)"""
        ...
        INSERT INTO {{EDW_DB_NAME}}.CIPINTEGRATIONODS.STG_CX_IB_EBH_HOTEL_RESV_LOAD
        (PLAYER_ID, RESERVATION_ID, CONFIRMATION_ID, START_DATE, END_DATE, CREATED_DTM, LAST_UPDATE_DTM, SITE_ID, SOURCE_SYSTEM_CODE, STATUS, PRECHECKIN, LAST_NAME, FIRST_NAME, EMAIL, 
         ...
         COMPANY_BOOKING, ROUTING_ROOM, SHARE_WITH_PARENT_ID, SPECIAL_REQUESTS, QLK_LOAD_TIMESTAMP)
        SELECT DISTINCT
        CAST(MBR.MEMBERSHIP_CARD_NO AS VARCHAR) AS PLAYER_ID,
        ...
        CASE WHEN RN.RESORT='EBH' THEN 1 WHEN RN.RESORT='BOS' THEN 2 ELSE 0 END AS SITE_ID,
        ...
        FROM 
        {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_RESV_DLT_EBH RN -- Orig view
        JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_ACTIVE_MEMBERSHIPS_EBH MBR ON RN.NAME_ID = MBR.NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_EXTERNAL_REFERENCES_EBH EXRE ON RN.RESV_NAME_ID = EXRE.RESV_NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_NAME_EBH NM ON MBR.NAME_ID = NM.NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_EMAIL_EBH NMEM ON MBR.NAME_ID = NMEM.NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_PHONE_EBH NMPH ON MBR.NAME_ID = NMPH.NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_RESV_DATA_FIELDS_EBH RDE ON RN.RESV_NAME_ID = RDE.RESV_NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_DEPOSIT_AMOUNT_EBH CA ON RN.RESV_NAME_ID = CA.RESV_NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_DEPOSIT_PAID_EBH CP ON RN.RESV_NAME_ID = CP.RESV_NAME_ID
        LEFT JOIN {{EDW_DB_NAME}}.CIPINTEGRATIONODS.VW_CX_IB_RESERVATION_SPECIAL_REQUESTS_EBH SR ON RN.RESV_NAME_ID = SR.RESV_NAME_ID
        """

        session.sql(loadRoomReservationsSql).collect()

        insert_log_query = f"INSERT INTO {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_LOG_TBL VALUES ('SP_CX_IB_EBH_HOTEL_RESV_LOAD', CURRENT_TIMESTAMP(),'Room reservations stage load completed')"
        ...
        getMaxUpdtTsSql = f"""SELECT COALESCE(MAX(LAST_UPDATE_DTM),TO_CHAR(CURRENT_TIMESTAMP(),'YYYY-MM-DDTHH24:MI:SS.000Z')) AS MAX_LAST_UPDATE_DTM 
                              FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.STG_CX_IB_EBH_HOTEL_RESV_LOAD"""
        getMaxUpdtTsSqlRslt = session.sql(getMaxUpdtTsSql).collect()
        maxUpdtTs = getMaxUpdtTsSqlRslt[0]['MAX_LAST_UPDATE_DTM']

        getRoomReservationsCntSql = f"SELECT COUNT(1) AS RESV_CNT FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.STG_CX_IB_EBH_HOTEL_RESV_LOAD"
        ...
        getRoomReservationsSql = f"SELECT * FROM {{EDW_DB_NAME}}.CIPINTEGRATIONODS.STG_CX_IB_EBH_HOTEL_RESV_LOAD"
        ...
        for resvRow in getRoomReservationsSqlRslt:
            specialRequests_raw = resvRow['SPECIAL_REQUESTS']
            if isinstance(specialRequests_raw, str):
                try:
                    specialRequests = json.loads(specialRequests_raw)
                    if not isinstance(specialRequests, list):
                        specialRequests = []
                except Exception:
                    specialRequests = []
            else:
                specialRequests = specialRequests_raw     
            if resvRow['CONFIRMATION_ID'] is not None:
                ...
                updateResvJson = {
                    "$set": {
                        ...
                        "room.special_requests":specialRequests,
        ...
        updateBatchSql = f"""UPDATE {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_HOTEL_RESV_LOAD_STATUS
                              ...
                              WHERE SITE_ID='EBH' and BATCH_STATUS = 'R'
                              """
...
def log_msg(message, session):
    insert_log_query = f"""INSERT INTO  {{EDW_DB_NAME}}.CIPINTEGRATIONODS.AUDIT_CX_IB_LOG_TBL (SP_NAME, LOG_TS, LOG_MSG)
                          VALUES ('SP_CX_IB_EBH_HOTEL_RESV_LOAD', CURRENT_TIMESTAMP(), '{message}')"""
    session.sql(insert_log_query).collect()
$$;
